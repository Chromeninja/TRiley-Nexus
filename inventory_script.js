...
const projects = fs
  ...
  .map((file) => {
    ...
    const problem =
      bodyLower.includes("## problem") || bodyLower.includes("### problem");
    const approach =
      bodyLower.includes("## approach") || bodyLower.includes("### approach");
    const outcome =
      bodyLower.includes("## outcome") || bodyLower.includes("### outcome");
  ...
  Object.keys(grouped)
    ...
    .forEach((org) => {
      ...
      grouped[org].forEach((p) => {
        console.log(
          `${p.filename} | ${p.title} | ${p.roleTitle} | ${p.category} | ${p.timeframe} | ${p.p}/${p.a}/${p.o}`,
        );
      });
    });
} catch (error) {
